...
- Exactly seven tools (`health_check`, `list_projects`, `list_servers`, `list_server_providers`, `list_source_controls`, `get_server`, `reboot_server`). `health_check` is a project-free probe that any valid token (read or write) may call. There are no legacy aliases and no generic dispatching tool.
