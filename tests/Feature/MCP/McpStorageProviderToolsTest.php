<?php

use App\Enums\UserRole;
use App\Models\Project;
use App\Models\StorageProvider;
use App\Models\User;
use Illuminate\Foundation\Testing\RefreshDatabase;
use Illuminate\Testing\TestResponse;
use Tests\Feature\MCP\Concerns\SpeaksMcp;

uses(RefreshDatabase::class, SpeaksMcp::class);

/**
 * Decode the first text content block of a tools/call result.
 *
 * @return array<string, mixed>
 */
function storagePayload(TestResponse $response): array
{
    return (array) json_decode((string) $response->json('result.content.0.text'), true);
}

test('list_storage_providers returns the caller global and in-scope providers with safe fields only', function () {
    /** @var StorageProvider $global */
    $global = StorageProvider::factory()->create([
        'user_id' => $this->user->id,
        'project_id' => null,
    ]);
    /** @var StorageProvider $scoped */
    $scoped = StorageProvider::factory()->create([
        'user_id' => $this->user->id,
        'project_id' => $this->user->current_project_id,
    ]);

    $plainToken = $this->user->createToken('mcp-read', ['read'])->plainTextToken;
    $this->mcpInitialize($plainToken);

    $response = $this->mcpCallTool('list_storage_providers');

    expect($response->json('result.isError'))->toBeFalse();

    $payload = storagePayload($response);

    $returnedIds = collect($payload['storage_providers'])->pluck('id')->toArray();
    expect($returnedIds)->toEqualCanonicalizing([$global->id, $scoped->id]);

    foreach ($payload['storage_providers'] as $provider) {
        expect(array_keys($provider))->toEqual([
            'id', 'project_id', 'global', 'name', 'provider', 'created_at', 'updated_at',
        ]);
    }

    // The global provider is advertised as global; the scoped one is not.
    $byId = collect($payload['storage_providers'])->keyBy('id');
    expect($byId[$global->id]['global'])->toBeTrue()
        ->and($byId[$global->id]['project_id'])->toBeNull()
        ->and($byId[$scoped->id]['global'])->toBeFalse()
        ->and($byId[$scoped->id]['project_id'])->toBe($this->user->current_project_id);
});

test('list_storage_providers omits providers owned by other users', function () {
    /** @var User $other */
    $other = User::factory()->create();
    $other->ensureHasDefaultProject();

    /** @var StorageProvider $othersProvider */
    $othersProvider = StorageProvider::factory()->create([
        'user_id' => $other->id,
        'project_id' => null,
    ]);

    /** @var StorageProvider $mine */
    $mine = StorageProvider::factory()->create([
        'user_id' => $this->user->id,
        'project_id' => $this->user->current_project_id,
    ]);

    $plainToken = $this->user->createToken('mcp-read', ['read'])->plainTextToken;
    $this->mcpInitialize($plainToken);

    $payload = storagePayload($this->mcpCallTool('list_storage_providers'));

    $returnedIds = collect($payload['storage_providers'])->pluck('id')->toArray();
    expect($returnedIds)->toEqual([$mine->id])
        ->and($returnedIds)->not->toContain($othersProvider->id);
});

test('list_storage_providers omits providers outside the token project scope but keeps global ones', function () {
    /** @var Project $scopedProject */
    $scopedProject = Project::factory()->create();
    $scopedProject->users()->create([
        'user_id' => $this->user->id,
        'role' => UserRole::ADMIN,
    ]);

    /** @var Project $otherProject */
    $otherProject = Project::factory()->create();
    $otherProject->users()->create([
        'user_id' => $this->user->id,
        'role' => UserRole::ADMIN,
    ]);

    /** @var StorageProvider $inScope */
    $inScope = StorageProvider::factory()->create([
        'user_id' => $this->user->id,
        'project_id' => $scopedProject->id,
    ]);
    /** @var StorageProvider $outOfScope */
    $outOfScope = StorageProvider::factory()->create([
        'user_id' => $this->user->id,
        'project_id' => $otherProject->id,
    ]);
    /** @var StorageProvider $global */
    $global = StorageProvider::factory()->create([
        'user_id' => $this->user->id,
        'project_id' => null,
    ]);

    // Token scoped to a single project: only that project's providers plus the
    // global ones are reachable through the token-project seam.
    $plainToken = $this->user->createToken('mcp-scoped', ['read', 'project:'.$scopedProject->id])->plainTextToken;
    $this->mcpInitialize($plainToken);

    $payload = storagePayload($this->mcpCallTool('list_storage_providers'));

    $returnedIds = collect($payload['storage_providers'])->pluck('id')->toArray();
    expect($returnedIds)->toEqualCanonicalizing([$inScope->id, $global->id])
        ->and($returnedIds)->not->toContain($outOfScope->id);
});

test('list_storage_providers requires a read-capable token', function () {
    $plainToken = $this->user->createToken('mcp-write-only', ['write'])->plainTextToken;
    $this->mcpInitialize($plainToken);

    $payload = storagePayload($this->mcpCallTool('list_storage_providers'));

    expect($payload['error']['code'])->toBe('forbidden_ability')
        ->and($payload['error']['message'])->toBeString();
});

test('list_storage_providers never leaks credentials or secret fields', function () {
    /** @var StorageProvider $provider */
    $provider = StorageProvider::factory()->create([
        'user_id' => $this->user->id,
        'project_id' => $this->user->current_project_id,
        'credentials' => ['api_key' => 'PROVIDER-SECRET-XYZ', 'token' => 'hunter2'],
    ]);

    $plainToken = $this->user->createToken('mcp-read', ['read'])->plainTextToken;
    $this->mcpInitialize($plainToken);

    $response = $this->mcpCallTool('list_storage_providers');

    expect($response->json('result.isError'))->toBeFalse();

    $rendered = (string) $response->json('result.content.0.text');
    $payload = json_decode($rendered, true);

    // Exact safe field set, nothing else.
    expect(array_keys($payload['storage_providers'][0]))->toEqual([
        'id', 'project_id', 'global', 'name', 'provider', 'created_at', 'updated_at',
    ]);

    // Secret poison and excluded columns never reach the wire.
    expect($rendered)->not->toContain('PROVIDER-SECRET-XYZ')
        ->not->toContain('hunter2')
        ->not->toContain('user_id')
        ->not->toContain('credentials')
        ->not->toContain('editable_data')
        ->not->toContain('provider_data')
        ->not->toContain('access_token');
});

test('tools/list exposes list_storage_providers with no input schema', function () {
    $plainToken = $this->user->createToken('mcp-discovery', ['read'])->plainTextToken;
    $this->mcpInitialize($plainToken);

    $tools = collect($this->mcpListTools()->json('result.tools'));
    $names = $tools->pluck('name')->values()->toArray();

    expect($names)->toContain('list_storage_providers');

    $tool = $tools->first(fn ($t) => $t['name'] === 'list_storage_providers');
    expect($tool['inputSchema']['properties'])->toBe([])
        ->and($tool['inputSchema'])->not->toHaveKey('required');
});
